...
const BigDecimalPoint = require('./BigDecimalPoint');

function main() {
    const a = new BigDecimal(-4);
    const b = new BigDecimal(4);
    const P = new BigDecimalPoint(new BigDecimal(-2), new BigDecimal(-2));
...
function applyDoubleAndAddMethod(P = new BigDecimalPoint(), k = new BigDecimal(0), a = new BigDecimal(0), b = new BigDecimal(0), precision = 100) {
    const kAsBinary = dec2bin(k.getValue());
    // console.log(`(${k.getValue()})10 = (${kAsBinary})2`);
    let outputPoint = new BigDecimalPoint(P.getPointX(), P.getPointY());
...
function applyPointAddition(P = new BigDecimalPoint(), Q = new BigDecimalPoint(), a = new BigDecimal(0), b = new BigDecimal(0), precision = 100) {
    ...
    const R = new BigDecimalPoint(x3, y3);
    return R;
}
